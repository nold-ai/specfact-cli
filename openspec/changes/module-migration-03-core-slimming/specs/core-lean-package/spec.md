...
Defines the behaviour of the slimmed `specfact-cli` core package after the 17 non-core module directories and the core auth module directory are removed from `src/specfact_cli/modules/` and `pyproject.toml`. Covers the installed wheel contents, the `specfact --help` output on a fresh install, category group mount behaviour when bundles are absent, and the bootstrap registration contract for the **3** core modules in this change (`init`, `module_registry`, `upgrade`).
...
This is a delta to the existing `command-registry` spec. The `bootstrap.py` behaviour changes from "register all bundled modules" to "register 3 core modules only."
...
- **THEN** exactly 3 call sites SHALL exist, one each for: `init`, `module_registry`, `upgrade`
