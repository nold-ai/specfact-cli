...
Same-repository identity is insufficient to identify token permissions: Dependabot and other restricted tokens can still receive HTTP 403. Await the optional comment API and catch only status 403 with a visible warning. Do not enumerate actors, elevate token permissions, ignore the entire job, or suppress other API errors. Supplemental tests execute the actual workflow JavaScript with deterministic API stubs for success, permission rejection and server error. The protected trigger proof remains frozen and separately validated.
