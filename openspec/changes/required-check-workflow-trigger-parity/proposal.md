...
Docs Review and Contract Validation are required checks, but their workflow-level pull-request path filters suppress runs on some documentation-only or ancestry-only changes. PR #732 cannot satisfy these checks for release PR #731. GitHub excludes manually dispatched job checks from required pull-request status evaluation, even when they pass on the same commit.
