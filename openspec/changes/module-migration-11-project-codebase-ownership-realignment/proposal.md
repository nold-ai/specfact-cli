...
- `specfact project import from-code` is the public path even though the command's primary input is a source codebase
...
- Reclassify code-first brownfield analysis behavior under `specfact code ...`, with `import from-code` treated as codebase-owned rather than project-owned.
...
- `import from-code` is defined as a codebase-owned command path in the target state, with a documented compatibility plan for any temporary `project` alias that remains during migration.
