...
from the validated module identity and version. It SHALL publish the tarball and
checksum from a commit already reachable from `dev` or `main`, download the
published tarball, and verify its checksum before advancing the bundled registry
snapshot. Existing release identities SHALL fail closed rather than overwrite an
...
- **AND** a source outside `dev` or `main`, a release collision, or a checksum mismatch SHALL stop publication before the snapshot changes.
