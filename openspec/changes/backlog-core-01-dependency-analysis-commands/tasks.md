...
- [ ] 0.1 Create module directory: `modules/backlog-core/`
- [ ] 0.2 Create `modules/backlog-core/module-package.yaml` with:
  - `name: backlog-core`, `version: 0.1.0`
  - `commands` list for all backlog + delta subcommands
  ...
  - `dependencies: []`
- [ ] 0.3 Create module source layout: `modules/backlog-core/src/backlog_core/__init__.py`, `main.py`
- [ ] 0.4 Register module in `src/specfact_cli/registry/bootstrap.py` for lazy loading (no changes to `cli.py`)

## 1. Phase 1: Backlog Dependency Analysis (v0.26.0)

### 1.1 Core Data Model: Provider-Agnostic Dependency Graph

- [ ] 1.1.1 Create `modules/backlog-core/src/backlog_core/graph/` directory structure
- [ ] 1.1.2 Implement `ItemType` enum (EPIC, FEATURE, STORY, TASK, BUG, SUB_TASK, CUSTOM) in `modules/backlog-core/src/backlog_core/graph/models.py`
...
- [ ] 1.2.1 Create `modules/backlog-core/src/backlog_core/graph/` directory structure for builder
- [ ] 1.2.2 Create template YAML files in `modules/backlog-core/src/backlog_core/resources/backlog-templates/`: `ado_scrum.yaml`, `ado_safe.yaml`, `github_projects.yaml`, `jira_kanban.yaml` with type_mapping, dependency_rules, status_mapping sections (bundled inside the module)
...
- [ ] 1.3.1 Implement `DependencyAnalyzer` class in `modules/backlog-core/src/backlog_core/analyzers/dependency.py` with `__init__()` method (add `@beartype` and `@icontract` decorators)
...
- [ ] 1.4.1 Define `BacklogGraphProtocol` in `modules/backlog-core/src/backlog_core/adapters/backlog_protocol.py` with `@runtime_checkable` Protocol: `fetch_all_issues(project_id: str, filters: dict | None = None) -> list[dict[str, Any]]`, `fetch_relationships(project_id: str) -> list[dict[str, Any]]`; add `@beartype` and `@icontract` decorators to the protocol interface
- [ ] 1.4.2 Register `BacklogGraphProtocol` with the bridge registry (arch-05) so it can be resolved at runtime without hard imports
- [ ] 1.4.3 Implement `BacklogGraphProtocol` in `modules/github-adapter/` (or wherever GitHubAdapter lives): add `fetch_all_issues()` and `fetch_relationships()` methods satisfying the protocol; register implementation with bridge registry
- [ ] 1.4.4 Implement `BacklogGraphProtocol` in `modules/ado-adapter/` (or wherever AdoAdapter lives): add `fetch_all_issues()` and `fetch_relationships()` methods; register implementation with bridge registry
- [ ] 1.4.5 Validate protocol conformance with `isinstance(adapter, BacklogGraphProtocol)` check in DependencyAnalyzer commands (fail fast with clear error if adapter doesn't support graph protocol)
- [ ] 1.4.6 Add unit tests for protocol conformance and bridge registry resolution in `modules/backlog-core/tests/unit/test_backlog_protocol.py`
...
- [ ] 1.5.1 Create `modules/backlog-core/src/backlog_core/commands/` directory structure
- [ ] 1.5.2 Implement `analyze_deps.py` with `analyze_deps()` command using typer (add `@beartype` decorator)
...
- [ ] 1.5.12 Create `backlog_app = typer.Typer(name="backlog", help="Backlog dependency analysis and sync")` in `modules/backlog-core/src/backlog_core/main.py`
- [ ] 1.5.13 Register `analyze_deps` and `trace_impact` commands to `backlog_app`
- [ ] 1.5.14 Declare `backlog` command group in `modules/backlog-core/module-package.yaml` — registry auto-discovers and lazy-loads; **no changes to `cli.py`**
...
- [ ] 2.2.1 Create `delta_app = typer.Typer(name="delta", help="Backlog delta analysis and impact tracking")` in `modules/backlog-core/src/backlog_core/commands/delta.py`; declare `delta` command group in `module-package.yaml`
...
- [ ] 2.2.10 Declare `delta` command group in `modules/backlog-core/module-package.yaml` alongside `backlog` commands — **no changes to `cli.py`**
...
- [ ] 3.1.1 Declare `backlog_core.backlog_config` schema extension on `ProjectMetadata` in `modules/backlog-core/module-package.yaml` (arch-07); access via `metadata.get_extension("backlog_core", "backlog_config")` — **no direct modification of `ProjectMetadata` model in core**
...
- [ ] 3.5.1 Add `backlog_config` section to `.specfact/spec.yaml` schema (project-level defaults for provider linking, type mapping, dependency rules, auto-sync)
- [ ] 3.5.2 Add `devops_stages` section to `.specfact/spec.yaml` schema for plan/develop/review/release/monitor stage definitions
- [ ] 3.5.3 Declare `backlog_core.backlog_graph` schema extension on `ProjectBundle` in `modules/backlog-core/module-package.yaml` (arch-07) — **no direct modification of `ProjectBundle` Pydantic model in core**
...
- [ ] 3.5.4 Implement `BacklogGraph` serialization to YAML/JSON (Pydantic model serialization) in `modules/backlog-core/src/backlog_core/graph/models.py`
- [ ] 3.5.5 Validate that schema extension registration via arch-07 works correctly at module load time
- [ ] 3.5.6 Add tests for schema extension access patterns in `modules/backlog-core/tests/unit/test_schema_extensions.py`
