...
| `range` | repository identity, seal-bound implementation-lineage origin commit/tree IDs, full head commit/tree IDs, and producer/policy/toolchain-bound ancestry attestation | complete immutable lineage-origin tree to descendant head tree across all successor seals | not representable and therefore absent | additions, deletions, and both rename endpoints retained | before/after modes and symlink target identity retained |

Every manifest record retains its change kind and byte-preserving path identity. Rename classification is deterministic under the bound producer, toolchain, and policy identities; consumers cannot reinterpret a delete/add pair under different rename settings. A final range whose base commit/tree differs from the seal-bound implementation-lineage origin is stale, and absent or invalid origin-to-head ancestry is unverifiable. Successor seals preserve that origin, so neither reapproval nor a caller can truncate the evaluated range by choosing a later reviewed snapshot.
