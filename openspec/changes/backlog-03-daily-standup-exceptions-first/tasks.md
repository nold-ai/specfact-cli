...
1. **Spec deltas** define behavior in `openspec/changes/backlog-03-daily-standup-exceptions-first/specs/daily-standup/spec.md`.
...
- [ ] 1.2 Create branch linked to #175: `gh issue develop 175 --repo nold-ai/specfact-cli --name feature/backlog-03-daily-standup-exceptions-first --checkout` (or `git checkout -b feature/backlog-03-daily-standup-exceptions-first` if no gh)
...
- [ ] 5.1 Commit and push: `git add .` then `git commit -m "feat(backlog): daily standup exceptions-first and --mode scrum|kanban|safe (fixes #175)"` and `git push origin feature/backlog-03-daily-standup-exceptions-first`
