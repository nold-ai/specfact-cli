...
- [ ] 2.1 Add focused cases for missing/empty/malformed/wrong-revision JUnit, duplicate selected outcomes within one execution unit, separate required matrix units/current attempts, exact selected outcomes (ordinary pass versus XFAIL, strict/non-strict XPASS, and empty `wasxfail` markers), and no promotion of local/self-declared metadata to CI authority; observe meaningful failures before code changes.
