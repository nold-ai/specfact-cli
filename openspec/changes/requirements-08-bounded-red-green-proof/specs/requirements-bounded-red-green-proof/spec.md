...
Core SHALL evaluate historical red-green proof against three explicit full Git identities: B, the pull-request merge base; R, an accepted red commit; and H, the evaluated head. It SHALL prove B is an ancestor of R and R is a strict ancestor of H. B..R SHALL contain no governed implementation-touchpoint change, and the complete R..H changed-path set SHALL be a subset of explicitly mapped implementation touchpoints.

The validator SHALL use Git path and rename facts only. It SHALL NOT discover imports, pytest plugins, configuration, data reads, aliases, mutations, namespaces, symlinks, or other runtime dependency closure.

#### Scenario: Declared implementation-only transition is eligible

- **GIVEN** valid full B, R, and H commits with B < R < H
- **AND** B..R changes tests/specifications but no governed implementation touchpoint
...
#### Scenario: Test, harness, policy, or unclassified change invalidates R
...
- **GIVEN** an eligible boundary and accepted exact selectors
...
- **AND** both result sets are retained for attestation.

#### Scenario: Replay cannot establish chronology

- **GIVEN** checkout, environment, collection, execution, or result identity fails at either endpoint
...
Core SHALL emit a content-addressed attestation binding B/R/H commits and trees, B..R and R..H path-set digests, mapping and plan digests, exact selectors, red and final JUnit digests, runner/toolchain/environment identities, policy identity, verifier identity and epoch, timestamps, and resource bounds.
...
#### Scenario: Attestation is complete and replayable

- **GIVEN** an eligible boundary and successful endpoint replay
- **WHEN** the attestation is built
- **THEN** every mandatory identity and digest is present
...
- **GIVEN** H changes the replay runner, workflow, policy, module fixture, attestation schema, or verifier identity
...
Missing or shallow history, invalid or abbreviated refs, unresolved merge base, checkout failure, changed undeclared paths, selector mismatch, missing artifacts, tool failure, timeout, environment mismatch, or verifier mismatch SHALL produce an explicit unproven result and non-zero strict exit after diagnostics are retained. None of these conditions may be represented as pass, skip, or no-impact.

#### Scenario: Mandatory fact is unavailable

- **GIVEN** at least one mandatory scope, identity, execution, or artifact fact cannot be established
