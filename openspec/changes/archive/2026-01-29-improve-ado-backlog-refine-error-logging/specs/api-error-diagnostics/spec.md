...
Define how SpecFact CLI captures, logs, and surfaces API errors (especially ADO PATCH failures) so that with `--debug` users see response body and patch paths, and without debug users see clear, actionable messages (e.g. missing field name and custom-mapping hint).
