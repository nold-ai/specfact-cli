...
The following are explicitly **not** in scope for this change; they are documented so stakeholders know where the boundary is:

- **Stale / at-risk items** (e.g. "no update in N days"): Not required. Many teams rely on "last updated" + blockers; no SHALL for at-risk flags or thresholds.
- **Sprint goal**: Not displayed or edited by this change; users see sprint goal in their board/sprint settings (document in user docs).
