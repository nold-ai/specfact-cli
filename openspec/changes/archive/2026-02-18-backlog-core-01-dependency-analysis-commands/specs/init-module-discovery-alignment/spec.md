# init-module-discovery-alignment Specification (Delta)

## Purpose

Ensures `specfact init` module discovery uses the same roots as command registration so workspace-level and custom modules appear in `--list-modules`, `--enable-module`, and `--disable-module`.
