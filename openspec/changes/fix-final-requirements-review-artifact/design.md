...
1. No Python targets: review succeeds, output remains `false`, and no report is
   expected or uploaded.
2. Python targets and successful review: output is `true`, and the report is
   required and uploaded.
3. Python targets and failed review: output is already `true`, so any produced
   report is retained; a missing report and the failed review both remain
   blocking.
