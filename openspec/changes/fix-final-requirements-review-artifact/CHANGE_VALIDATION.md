...
- Focused passing-after evidence: passed (`3 passed` mapped; `58 passed` full
...
- The test-authored local Code Review reported three generic-name warnings at
  lines 164, 170, and 177 of `test_trustworthy_green_checks.py`. Those selectors
  are unchanged baseline tests introduced and approved by #708; they are not in
  the #710 diff. Renaming them would invalidate #708 mapping/proof identities and
  expand this workflow-only fix, so they remain a documented pre-existing
  exception rather than being modified here.
