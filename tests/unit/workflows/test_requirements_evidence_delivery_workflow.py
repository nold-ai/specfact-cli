...
def test_requirements_evidence_workflow_supports_verified_amendment_cycles() -> None:
    """A later review fix must run red-to-green on the existing pull request."""
    workflow = REPO_ROOT / ".github" / "workflows" / "requirements-evidence.yml"
    parsed = yaml.load(workflow.read_text(encoding="utf-8"), Loader=yaml.BaseLoader)
    cycle_step = _step_by_name(parsed, "Locate verified amendment cycle base")
    run_evidence = _step_by_name(parsed, "Run Requirements evidence gate")
    command = run_evidence["run"]
    assert isinstance(command, str)
    run_environment = run_evidence["env"]
    assert isinstance(run_environment, dict)

    cycle_command = cycle_step["run"]
    assert isinstance(cycle_command, str)
    assert all(
        fragment in cycle_command
        for fragment in (
            "requirements_cycle_base.py",
            "-f status=success",
            'git merge-base --is-ancestor "$head_sha" "$current_head"',
    ...
    assert run_environment["EVIDENCE_CYCLE_BASE"] == "${{ steps.cycle-base.outputs.source-ref }}"
    assert 'cycle_base_ref="${EVIDENCE_CYCLE_BASE:-origin/${EVIDENCE_BASE_BRANCH}}"' in command
    assert 'git diff --name-status -z --find-renames "$cycle_base_ref...HEAD"' in command
    assert '--cycle-authority "$cycle_authority"' in command
    assert "--cycle-base-ref" not in command
