...
def test_docs_gate_fails_before_pytest_without_explicit_context(
    ...
    workflow = yaml.safe_load((REPO_ROOT / ".github/workflows" / filename).read_text())
    matching_steps = [step for step in workflow["jobs"][job]["steps"] if step.get("name") == step_name]
    assert len(matching_steps) == 1, f"Required documentation context gate is missing: {step_name}"
